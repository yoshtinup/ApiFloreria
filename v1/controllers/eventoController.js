...
export const eventoController = {
    getAllProducts : async (req, res) => {
        const allProducts = await eventoService.getAllProducts();
        res.status(200).send(allProducts);
    },
    getOneProduct : async (req, res) => {
        let id = req.params.id;
        const product = await eventoService.getOneProduct(id);
        res.status(200).send({ status: "OK", data: product });
    },
    createNewProduct : async (req, res) => {
        const data = req.body;
        const createProduct = await eventoService.createNewProduct(data);
    ...
    updateOneProduct: async (req, res) => {
        ...
        try {
            const updatedProduct = await eventoService.updateOneProduct(id, data);
            if (updatedProduct) {
    ...
    deleteOneProduct : async (req, res) => {
        let id = req.params.id;
        const deleteProduct = eventoService.deleteOneProduct(id);
